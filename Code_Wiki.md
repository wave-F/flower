...
3. 底部区域现在是 `Energy Meter`，会累计本局内被消除的非目标能量。
4. 如果本次消除里包含目标种类，会先用于补足当前还没完成的目标；只有超出目标需求的同种 tile 才会转成能量。
5. 每累计 `20` 个有效能量，就会自动在棋盘上随机挑一个非特殊普通格，替换生成一个 `蜂巢 Hive`。
...
- 新增底部 `collection-tray-panel`，当前作为能量回收面板使用。
- 面板内部显示一条 `Energy Meter`，用于展示距离下一个自动蜂巢还差多少消除量。
...
- 点击风车 / 蜂巢：继续沿用特殊块触发逻辑。
- 每个被移除的非目标 tile 都会累计到底部能量条里。
- 如果移除的是目标种类，会先扣当前剩余目标需求；只有超出的那部分才会进入能量条。
- 能量累计到 `20` 时，会自动生成一个 `蜂巢 Hive`。
- 自动蜂巢会替换棋盘上一个随机的、非特殊的普通 tile；优先替换花，若没有花才会回退到 `grass`。
