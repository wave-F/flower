...
5. 每累计 `15` 点有效能量，就会自动在棋盘上随机挑一个非特殊普通格，替换生成一个 `光球 Lightball`。
...
- 当前能量规则为：只统计特殊道具触发的充能值；总能量 = 基础能量 + 连携奖励，其中连携奖励只在同一次特殊道具使用触发了 `2` 个及以上道具时生效，数值等于这条特殊道具链一共触发了多少个道具。
- 每个特殊充能粒子到达时，`Energy Meter` 会先做实时预览；等本次所有粒子飞完，并且下落与后续消除都结束后，才会统一结转真实能量并生成 `光球 Lightball`。
- 如果一次结算会生成多个光球，能量条会先保持在 `15 / 15` 的满槽状态，再以固定 `300ms` stagger 依次启动多颗光球飞行，直到最后一颗飞完后再播放一次平滑回落动画，落到剩余进度。
...
- 点击底部能量槽会弹出一条短 tips，提示“触发特殊道具，充能获得彩虹花！”。
...
- 当前基础值：`风车 = +2`，`大风车 = +2`，`炸弹 = +5`，`光球 = +0`。
- 连携奖励：只看“同一次特殊道具使用一共触发了多少个特殊道具”，并且至少要触发 `2` 个道具才会生效。
- 例如：只触发自己时额外 `+0`；A 触发 B 时额外 `+2`；A 触发 B、C 时额外 `+3`。
- 下落补位后形成的普通自动消除，不算这里的“连锁触发”，只算后续消除。
- 能量累计到 `15` 时，会自动生成一个 `光球 Lightball`。
...
- `特殊充能表现 Special Charge FX`
  - 风车 / 大风车 / 炸弹在自己的触发动画完成后，会从 origin 位置发出能量光点，沿曲线飞向底部 `Energy Meter`；总粒子数与最终充能值保持一致。
  - 每个光点到达时，`recycleChargePreview` 会立即 `+1`，让进度条先做实时预览，再在 `resolveRecycleProgress()` 中统一结转成真实能量。
  - 当前会等本次所有粒子飞完，并且下落与后续消除全部结束后，再统一飞出光球奖励；如果要飞多颗，会先锁定 `15 / 15` 满槽，再按 `300ms` stagger 并发启动，并通过预占位避免多个光球选到同一个格子，最后用一段短动画落到余量。
