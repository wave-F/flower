...
3. 底部区域现在是 `Energy Meter`，采用“非目标普通消除充能 + 大量连消额外奖励”的组合规则。
4. 目标 tile 仍然优先用于推进关卡目标；只有本次已不再需要计入目标的普通 tile，才会转成能量。
...
- 当前能量规则为：本次被移除的普通 tile 如果没有被计入未完成目标，就会进入充能；特殊 tile 自身不直接提供充能；同一步里若普通消除总数达到 `20+`，还会额外触发一次连击奖励。
...
- 特殊 tile 自身不提供底部能量；光球锁定并清掉的目标 tile 也不会反向给自己充能，避免循环。
