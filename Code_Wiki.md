...
1. 玩家点击普通花或 `grass` 时，都会把它收进底部的 `7` 个槽位之一。
2. 棋盘空位会继续执行下落与顶部补位，并在形成 `3+` 同色正交连通块时自动消除与连锁。
3. 槽位不再做 `3` 合 `1`，改为累计收集：累计到 `5` 生成风车，累计到 `10` 生成蜂巢。
4. 生成出的风车 / 蜂巢会进入槽位，且可以被拖回棋盘使用；槽位中的 tile 本身不计入 `goals`。
...
- 点击普通花或 `grass`：先飞入底部槽位，再让棋盘执行下落、补位和后续自动连锁。
- 点击风车 / 蜂巢：继续沿用旧的特殊块触发逻辑，不进入底部槽位。
- 每收进一个普通 tile，槽位累计值 `+1`；累计到 `5` 时生成一个风车道具，累计到 `10` 时生成一个蜂巢道具，并重置这一轮累计。
- `grass` 不能直接拖回棋盘；槽位里的普通花、风车、蜂巢都可以拖拽回棋盘。
- 槽位里的普通花 / 风车 / 蜂巢拖到普通花/草格时，会拔掉原 tile，种下新内容，并立即触发棋盘自动结算；这次种植会消耗 `1` 步。
