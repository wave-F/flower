...
1. 玩家点击一个 tile，把它收进底部的 `7` 个槽位之一。
2. 被收走的棋子会让棋盘继续执行下落与顶部补位。
3. 如果下落后形成 `3+` 同色正交连通块，棋盘会继续自动消除与连锁。
4. 槽位中的 tile 目前不计入 `goals`，主要用于验证 Match3D 风格的点击收集手感。
...
- 点击普通棋盘 tile：先飞入底部槽位，再让棋盘执行下落、补位和后续自动连锁。
