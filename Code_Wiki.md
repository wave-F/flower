...
3. 底部区域现在是 `Energy Meter`，采用“按普通花数量充能 + 大量连消额外奖励”的组合规则。
4. 目标 tile 仍然优先用于推进关卡目标；只有本次已不再需要计入目标的普通 tile，才会转成能量。双光球本体清掉的普通花默认不充能，但如果这些花落在被双光球带爆的风车 / 炸弹作用范围内，则会改归因为特殊道具清除并参与充能。
5. 每累计 `100` 点有效能量，就会自动在棋盘上随机挑一个非特殊普通格，替换生成一个 `光球 Lightball`。
...
- 当前能量规则为：本次被移除的普通 tile 如果没有被计入未完成目标，就会进入充能；特殊 tile 自身不直接提供充能，真正记能量的是它们作用范围里被移除的普通花数量；同一步里若普通消除总数达到 `20+`，还会额外触发一次连击奖励。
- 普通移除飞向底部 `Energy Meter` 时会先做实时预览；等本次飞行动画、下落与后续消除都结束后，才会统一结转真实能量并生成 `光球 Lightball`。
- 连击奖励会在 HUD 的连击提示隐藏时结算，按 `count / 2` 转成额外能量，并拆成若干飞向能量槽的光粒；如果这一步已经完成关卡，则这笔延后奖励会被直接丢弃，不再飞光球也不再加能量。
- 如果一次结算会生成多个光球，能量条会先保持在 `100 / 100` 的满槽状态，再以固定 `300ms` stagger 依次启动多颗光球飞行，直到最后一颗飞完后再播放一次平滑回落动画，落到剩余进度。
- 当 `Energy Meter` 处于满能量待发状态时，会保持一个轻微的待机闪烁，提示玩家奖励即将飞出。
- 风车 / 光球点击结算逻辑仍保留并继续作为特殊块默认入口。
- 双风车合体入口已关闭；相邻风车现在只会按普通特殊连锁继续触发，不再生成大风车。
- 双光球当前仍保持“全盘核爆”的主视觉，但如果冲击波命中场上已有的风车 / 炸弹，这些特殊块会改为原地触发自己的短版爆发效果，而不是像普通花一样直接被吹飞。
...
- 风车特效时序配置。
- 包含加速、SVG 风带、飞花、淡出，以及风团厚度/弯曲/分散/风屑等主风形参数；当前主流程里风吹到目标后会直接进入既有消除表现，不再额外播放命中抖动。
...
- 双光球当前会读取 `DUAL_LIGHTBALL_TIMINGS`，按“公转 -> 停顿 -> 对撞 -> 爆炸”的节奏播放；冲击波命中特殊道具时，会转接到该道具自己的爆发 effect，而不是统一按普通 tile 飞散处理。
...
- 面板内部显示一条 `Energy Meter`，用于展示距离下一个自动光球还差多少普通消除有效充能。
- 点击底部能量槽会弹出一条短 tips，提示“消除非目标棋子可充能，攒满生成光球！”。
...
- 点击风车 / 光球：继续沿用特殊块触发逻辑。
- 普通点击消除、普通三消、普通自动连锁，都会把其中“未被目标占用”的普通 tile 计入能量。
- 如果某个 tile 对应的关卡目标还没满，它会优先飞向目标栏，不计入能量；只有超出目标后的同类 tile 才会改为充能。
- 特殊 tile 自身不提供底部能量；风车 / 炸弹只通过自己作用范围里被移除的普通花数量来间接提供能量。
- 双光球本体直接清掉的普通花默认不计底部能量；但如果某朵普通花同时落在被双光球带爆的风车 / 炸弹作用范围里，则会优先归因给该特殊道具并照常充能。
- 同一步中被移除的普通 tile 总数达到 `20` 后，会显示连击 toast；toast 隐藏时把累计数量按 `count / 2` 转成额外能量奖励。
- 这份连击奖励是异步延后结算的；如果该步消除已经完成关卡，则会直接取消，不再飞向能量槽。
- 能量累计到 `100` 时，会自动生成一个 `光球 Lightball`。
...
- `光球 Lightball`
  ...
  - 统一消除后目标花会直接飞向顶部目标栏；剩余未计入目标的普通花会按普通结算规则参与能量统计。
  - 随后清除全盘该花色，并进入正常的掉落、补位与后续自动连锁。
  - 当前版本不会被风车等其他特殊块被动触发；只响应玩家主动点击。
  - 光球本身不参与反向充能，避免形成自循环。
  - 双光球共鸣 `dual lightball` 是单独的一套效果：它会核爆全盘，并且会把场上已有的风车 / 炸弹带入自己的冲击波结算；这些被带爆道具覆盖到的普通花，会被重新归因到道具侧用于充能统计。

- `特殊充能表现 Special Charge FX`
  - 普通移除与连击奖励都会向底部 `Energy Meter` 发出光点；前者用于本次消除的基础充能，后者用于 `20+` 大量连消的额外奖励。
  - 每个光点到达时，`recycleChargePreview` 会立即增长，让进度条先做实时预览，再在 `resolveRecycleProgress()` 中统一结转成真实能量。
  - 当前会等本次所有粒子飞完，并且下落与后续消除全部结束后，再统一飞出光球奖励；如果要飞多颗，会先锁定 `100 / 100` 满槽，再按 `300ms` stagger 并发启动，并通过预占位避免多个光球选到同一个格子，最后用一段短动画落到余量。
  - 光球奖励飞向棋盘时，当前会直接使用道具自身图标，放慢飞行时长，并让图标在整段飞行过程中持续旋转。
  - 关卡完成后的收尾阶段如果已经判定过关，会主动丢弃尚未结算的连击奖励，避免结算结束后再补飞光球或补记能量。
