...
- 如果一次结算会生成多个光球，能量条会先保持在 `15 / 15` 的满槽状态，再以较短间隔依次飞出多颗光球，直到最后一颗飞完后再播放一次平滑回落动画，落到剩余进度。
...
- `特殊充能表现 Special Charge FX`
  ...
  - 当前会等本次所有粒子飞完，并且下落与后续消除全部结束后，再统一飞出光球奖励；如果要飞多颗，会先锁定 `15 / 15` 满槽，再逐颗飞出，最后用一段短动画落到余量。
