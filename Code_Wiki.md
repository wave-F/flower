...
1. 玩家点击普通花时，会直接把它拔掉丢弃。
2. 玩家点击 `grass` 时，才会把它收进底部的 `7` 个槽位之一。
3. 棋盘空位会继续执行下落与顶部补位，并在形成 `3+` 同色正交连通块时自动消除与连锁。
4. 槽位中的 tile 目前不计入 `goals`，主要用于承载草合成与回种玩法。
...
- 点击普通花：直接拔掉丢弃，再让棋盘执行下落、补位和后续自动连锁。
- 点击 `grass`：先飞入底部槽位，再让棋盘执行下落、补位和后续自动连锁。
