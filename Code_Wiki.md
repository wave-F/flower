...
当前核心玩法不是传统“交换两个相邻块”，而是一个正在迭代中的点击收集原型：

1. 玩家点击普通花或 `grass` 时，都会先把它收进底部的 `7` 个槽位之一。
2. 若槽位已满，再点击新 tile 时会先移除第 `1` 个槽位，再把新 tile 追加到最后一个槽位。
3. 棋盘空位会继续执行下落与顶部补位，并在形成 `3+` 同色正交连通块时自动消除与连锁。
4. 槽位中的 tile 目前不计入 `goals`，主要用于承载槽位合成与回种玩法。
...
- 当前主点击流程已切到：点击普通 tile -> 飞入底部 `7` 槽 -> 棋盘下落补位 -> 自动合成与连锁。
- 风车 / 蜂巢点击结算逻辑仍保留并继续作为特殊块默认入口。
...
#### `index.html` / `styles.css` 中新增的 Tray UI
- 新增底部 `collection-tray-panel`，固定渲染 `7` 个收集槽位。
- 槽位采用独立卡片样式，点击棋盘后 tile 会飞入对应槽位。
...
- 点击普通花或 `grass`：都会先飞入底部槽位，再让棋盘执行下落、补位和后续自动连锁。
- 点击风车 / 蜂巢：继续沿用旧的特殊块触发逻辑，不进入底部槽位。
- 槽位里只有 `3` 个 `grass` 会触发特殊合成：若当前棋盘仍存在普通花，则随机合成其中一种花；若当前棋盘已没有普通花，则直接清掉这 `3` 个杂草。
- 花不会参与槽位合成。
- 槽位里的花可以拖拽回棋盘：拖到普通花/草格时，会拔掉原 tile，种下新花，并立即触发棋盘自动结算；这次种植会消耗 `1` 步。
- 当前 `7` 槽放满后不会失败；再点击新 tile 时，会先移除最前面的槽位，再把新 tile 放进最后一个槽位。
